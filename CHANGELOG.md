...
- Descriptors: added Bachata, Uptempo Hardcore, Digicore, Glitchcore, Psychedelic Techno, Barcarola, Tribal House, Acid Techno, Hyperpop, Jersey Club, Muiñeira, Alalá, Afro-Rock, Afrobeat, Classical Crossover, Corrido, Drill, Bubblegum Bass, Deconstructed Club, Big Room House and Corrido Tumbado styles.
...
- Debug: additional checks at startup.
### Changed
- Recipes: improvements to the recipes menu for customizable button, which now allows to directly edit recipes as submenu entries. Added multiple entries to hide/show recipes, clone, remove or even reset default ones.
...
- Smart shuffle: now handles multi-value tags without joining them. i.e. it will try to scatter the most frequent tag value for every track on the entire output. For ex. for a track with 2 artists, the one which appears more in the entire output will be preferred.
