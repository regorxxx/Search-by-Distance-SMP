...
const Selection = (function () {
	...
	function _selection() {
		...
		function dictionaryButton() {
			...
			const url = Object.hasOwn(this, lanKey) ? this[lanKey] : this.enURL;
			popupWindow(url + encodeURIComponent(text));
			return false;
		}

		function validateDictValue(text) {
			var values = text.split(' ').filter((v) => v !== '');
		...
		function config(options) {
			Object.keys(options).forEach((option) => {
				if (Object.hasOwn(menu, option)) {
					menu[option] = typeof options[option] === 'undefined' ? menu[option] : options[option];
