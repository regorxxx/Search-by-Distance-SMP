...
/* global music_graph_descriptors_countries:readable, music_graph_descriptors_culture:readable, round:readable */
...
function getCountryISO(country, bAlpha2 = false) {
	let isoCode = '';
	if (country && country.length) {
		if (isoCoordinates.has(country)) {
			isoCode = country;
		} else if (alpha2toAlpha3.has(country.toUpperCase())) {
			isoCode = alpha2toAlpha3.get(country.toUpperCase());
		} else {
...
const isoMapRev = new Map(Array.from(isoMap, (arr) => arr.reverse()));
...
const alpha2toAlpha3 = new Map(Array.from(alpha3toAlpha2, (arr) => arr.reverse()));
...
const nameReplacersRev = new Map(Array.from(nameReplacers, (arr) => arr.reverse()));
...
const nameShortRev = new Map(Array.from(nameShort, (arr) => arr.reverse()));
