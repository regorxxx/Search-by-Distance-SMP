...
try {window.DefinePanel('Search by Distance Buttons', {author:'xxx'});} catch (e) {/* console.log('Search by Distance (DYNGENRE) Buttons loaded.'); */} //May be loaded along other buttons
...
addButton({
	'Search by Distance (D) nearest tracks': new themedButton({x: 0, y: 0, w: 106, h: 22}, 'Nearest Tracks', function () {
		let t0 = Date.now();
		let t1 = 0;
	...
	'Search by Distance (D) similar tracks': new themedButton({x: 0, y: 0, w: 103, h: 22}, 'Similar Tracks', function () {
		let t0 = Date.now();
		let t1 = 0;
	...
	'Search by Distance (D) similar genres': new themedButton({x: 0, y: 0, w: 103, h: 22}, 'Similar Genres', function () {
		let t0 = Date.now();
		let t1 = 0;
	...
	'Search by Distance (D) similar mood': new themedButton({x: 0, y: 0, w: 103, h: 22}, 'Similar Mood', function () {
		let t0 = Date.now();
		let t1 = 0;
